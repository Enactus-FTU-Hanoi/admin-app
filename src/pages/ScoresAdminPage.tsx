...
type Score = { id: string; member_id: string; member_name?: string; category: string; score: number; period: string; note?: string; created_at: string }
type Member = { id: string; name: string }

const CATEGORIES = ['Tham dự họp', 'Hoàn thành task', 'Đóng góp dự án', 'Kỷ luật', 'Sáng kiến', 'Khác']
...
export function ScoresAdminPage() {
  ...
  const [form, setForm]       = useState({ member_id:'', category:CATEGORIES[0], score:10, period:'', note:'' })
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setModal(false)
      const s = await api<Score[]>('/scores/all')
      setScores(s)
  ...
  return (
    <div>
      <div className="row-between" style={{ marginBottom: 18 }}>
        ...
        <button className="btn btn-primary btn-sm" onClick={() => setModal(true)}>+ Chấm điểm</button>
      </div>

      <div className="table-wrap">
        <table>
          <thead><tr>
            <th>Thành viên</th><th>Kỳ</th><th>Hạng mục</th><th>Điểm</th><th>Ghi chú</th><th>Ngày</th><th></th>
          </tr></thead>
          <tbody>
            {filtered.length === 0
              ...
              : filtered.map(s => (
                <tr key={s.id}>
                  ...
                  <td>{s.category}</td>
                  <td>
                    <span style={{ fontWeight:700, color: s.score >= 0 ? 'var(--green)' : 'var(--red)' }}>
                      {s.score > 0 ? '+' : ''}{s.score}
                    </span>
                  </td>
                  ...
                  <td>
                    <button className="btn btn-sm" style={{background:'#FEF2F2',color:'#B91C1C',border:'1px solid #FECACA'}} onClick={() => del(s.id)}>Xoá</button>
                  </td>
                </tr>
              ))}
          </tbody>
            ...
            <form onSubmit={save}>
              <div className="modal-body">
                ...
                <div className="g2">
                  ...
                  <div className="form-group">
                    ...
                    <input className="input" type="number" required value={form.score} onChange={e => setForm(f=>({...f,score:+e.target.value}))} />
                  </div>
                </div>
                <div className="form-group">
              ...
              <div className="modal-foot">
                ...
                <button type="submit" className="btn btn-primary" disabled={saving}>
                  {saving ? <><div className="spinner" style={{width:15,height:15,borderWidth:2}} /> Lưu...</> : 'Lưu điểm'}
                </button>
              </div>
            </form>
          </div>
